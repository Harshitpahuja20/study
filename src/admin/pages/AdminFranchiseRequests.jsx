...
import {
  deleteFranchiseRequests,
  getFranchiseRequests,
} from "../services/adminFranchiseRequests.service";

const AdminFranchiseRequests = () => {
  ...
  const fetchData = async (page) => {
    setDataLoading(true);
    const response = await getFranchiseRequests(page, filters);
  ...
  const handleDelete = async () => {
    try {
      if (selectedId) {
        await deleteFranchiseRequests(selectedId).then((response) => {
          if (response.data.status) {
                ...
                {tableData?.length > 0 ? (
                  tableData.map((data, index) => (
                    <tr key={index}>
                      ...
                      <td className=" py-3">
                        {data?.instituteName} <br /> {data?.directorName}
                      </td>
                      <td className=" py-3">
...
export default AdminFranchiseRequests;
